...
"""
Description:
    This is an association table which serves as a request table for a user to be able receive and accept or decline a project request.
    The user can then be added to the project through the association_user_project table.
...
association_project_request = db.Table(
    "association_project_request",
    db.metadata,
...
"""
Description:
    Many to many circular relationship on users which serves as a friend request to be able
    to store friend requests whilst the user hasnt accepted yet. As soon as the user accepts
    the request the row is deleted and added to association_user_friendlist. On the contrary
    if the user declines the request the row is simply deleted.
"""
association_friend_request = db.Table(
    "association_friend_request",
    db.metadata,
    db.Column("user_id", db.ForeignKey("user.id"), primary_key=True),
...
"""
Description:
    Many to many circular relationship on users which serves as a friend list to be able
    to add other users to a project.
"""
association_user_friendlist = db.Table(
    "association_user_friendlist",
    db.metadata,
    db.Column("user_id", db.ForeignKey("user.id"), primary_key=True),
    db.Column("friend_id", db.ForeignKey("user.id"), primary_key=True),
)
...
# date pas sûre
    ...
    sprint1 = Sprint(
                    creation = datetime.strptime('06/06/2021', '%d/%m/%Y').date(),
                    status = 1,
                    project_id = 1
                    )

    sprint2 = Sprint(
                    creation = datetime.strptime('06/08/2021', '%d/%m/%Y').date(),
                    status = 1,
                    project_id = 1
                    )
    
    sprint3 = Sprint(
                    creation = datetime.strptime('06/10/2021', '%d/%m/%Y').date(),
                    status = 0,
                    project_id = 1
                    )
    ...
    for i in range(20):
        if i%4 == 0:
            task = Task(name="test"+str(i), description = "test %d"%i, creation = datetime.strptime('06/06/2021', '%d/%m/%Y').date(), status = 2, pokerScore = 1, sprint_id = 1, project_id  = 1)
        elif i%4 == 1:
            task = Task(name="test"+str(i), description = "test %d"%i, creation = datetime.strptime('06/08/2021', '%d/%m/%Y').date(), status = 0, pokerScore = 1, project_id  = 1)
        elif i%4 == 2:
            task = Task(name="test"+str(i), description = "test %d"%i, creation = datetime.strptime('06/10/2021', '%d/%m/%Y').date(), status = 2, pokerScore = 1, sprint_id = 2, project_id  = 1)
        else:
            task = Task(name="test"+str(i), description = "test %d"%i, creation = datetime.strptime('06/12/2021', '%d/%m/%Y').date(), status = 0, pokerScore = 1, sprint_id = 3, project_id  = 1)
